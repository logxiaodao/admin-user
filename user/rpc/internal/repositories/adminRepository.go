...
type AdminRepository interface {
	PagingQuery(ctx context.Context, req *user.GetAdminRequest) (rsp user.GetAdminResponse, err error)
	FindAdminByIdList(ctx context.Context, idList []string) (roleList []*user.AdminInfo, err error)
	AddAdmin(ctx context.Context, req *user.AddAdminRequest) (err error)
	EditAdmin(ctx context.Context, req *user.EditAdminRequest) (err error)
	DeleteAdmin(ctx context.Context, req *user.DeleteAdminRequest) (err error)
...
// PagingQuery 分页查询
func (a *adminRepository) PagingQuery(ctx context.Context, req *user.GetAdminRequest) (rsp user.GetAdminResponse, err error) {
	// db 实例
...
func (a *adminRepository) FindAdminByIdList(ctx context.Context, idList []string) (userList []*user.AdminInfo, err error) {
	// db 实例
	db := a.db.WithContext(ctx)
...
// AddAdmin 单条插入
func (a *adminRepository) AddAdmin(ctx context.Context, req *user.AddAdminRequest) (err error) {
...
// EditAdmin 单条修改
func (a *adminRepository) EditAdmin(ctx context.Context, req *user.EditAdminRequest) (err error) {
	// db 实例
	tx := a.db.WithContext(ctx).Begin()
	...
	for _, v := range req.RoleIdList {
		adminUserHasRoles = append(adminUserHasRoles, model.AdminUserHasRole{UserID: uint(req.Id), RoleID: uint(v)})
	}
...
// DeleteAdmin 删除 支持批量
func (a *adminRepository) DeleteAdmin(ctx context.Context, req *user.DeleteAdminRequest) (err error) {
	// db 实例
	tx := a.db.WithContext(ctx).Begin()
