...
func (t *token) GetToken(ctx context.Context) (string, error) {
	...
	incorrectToken, expiredToken := errorx.CodeMessage[errorx.IncorrectToken], errorx.CodeMessage[errorx.ExpiredToken]
	...
	// 验证token是否在redis黑名单
	if dataSources.RedisClient.SIsMember(config.DefaultTokenRedisKey+":"+time.Now().Format("2006-01-02"), md.Get("authorization")[0]).Val() {
...
func (t *token) ParseHS256Token(tokenString string) (jwt.MapClaims, error) {
	...
	incorrectToken := errorx.CodeMessage[errorx.IncorrectToken]
...
func (t *token) GetTokenClaims(ctx context.Context) (jwt.MapClaims, error) {
	...
	if _, ok := userInfo["exp"]; ok {
		if int64(util.InterfaceToUint(userInfo["exp"])) < time.Now().Unix() {
...
func (t *token) SetContext(ctx context.Context, isSuperAdmin bool) (newCtx context.Context, err error) {
	...
	if isSuperAdmin {
		// 拦截非超管
		permissionDenied := errorx.CodeMessage[errorx.PermissionDenied]
		if _, ok := userInfo["isSuperAdmin"]; !ok {
			return nil, errors.New(permissionDenied.Error())
		}
		if util.InterfaceToUint(userInfo["isSuperAdmin"]) != uint(1) {
			return nil, errors.New(permissionDenied.Error())
		}
	}
