...
type AccountRepository interface {
	FindOneByAccount(ctx context.Context, account string, platformId uint) (rsp model2.AdminUser, err error)
	FindOneById(ctx context.Context, userId uint) (rsp model2.AdminUser, err error)
...
func (a *accountRepository) FindOneByAccount(ctx context.Context, account string, platformId uint) (rsp model2.AdminUser, err error) {
	// db 实例
...
func (a *accountRepository) FindOneById(ctx context.Context, userId uint) (rsp model2.AdminUser, err error) {
	// db 实例
	db := a.db.WithContext(ctx)
...
func (a *accountRepository) CheckPermission(ctx context.Context, userId uint, httpMethod, httpPath string) (isPass bool, err error) {
	...
	if adminUser.IsSuperAdmin == uint8(1) { // 管理每个平台的超管都拥有用户微服务的权限
		err = db.Model(model2.AdminAPI{}).
			Where(model2.AdminAPIColumns.PlatformID+" in ? ", []uint{adminUser.PlatformID, uint(config2.InitRouteData.PlatformId)}).
